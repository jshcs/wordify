...
def make_cumm_embeddings_with_spacy(input_sentence,nlp,method='additive'):
	input_tokens=[unicode(w,'utf-8') for w in input_sentence.split() if w not in SWORDS]
	N=50
	if method=='additive':
		res_embedding=np.zeros(N)
		for i in input_tokens:
			if i not in nlp.vocab:
				continue
			res_embedding+=nlp.vocab.get_vector(i)
	elif method=='multiplicative':
		res_embedding=np.ones(N)
		for i in input_tokens:
			if i not in nlp.vocab:
...
def cosine_sim(word_embedding1,word_embedding2):
	...
	#return n/(d1*d2)
	return 1-scipy.spatial.distance.cosine(word_embedding1,word_embedding2)
...
def get_most_similar_with_spacy(input_embedding,nlp,vocab,return_length,op='additive'):
	cs_list=[]
	for v in vocab:
		cs_list.append((v,cosine_sim(input_embedding,nlp.vocab.get_vector(unicode(v,'utf-8')))))
	sorted_cs_list=sorted(cs_list,key=lambda tup:tup[1],reverse=True)
	#return sorted_cs_list[:return_length]
...
def eval_method_with_spacy(test_set,vocab,nlp,recall_count,method='additive'):
	hits=0
	N=len(test_set)
	cnt=0
	result_list=[]
	...
	for ip in test_set:
		...
		target_word=ip[0]
		input_sentence=ip[1]
		input_embedding=make_cumm_embeddings_with_spacy(input_sentence,nlp,method)
		most_similar_list=get_most_similar_with_spacy(input_embedding,nlp,vocab,recall_count,op=method)
		result_list=get_list_from_tuple(most_similar_list)
		if target_word in result_list[:recall_count]:
			hits+=1
		if target_word in result_list:
			rank_list.append(result_list.index(target_word))
			print "Rank of %s is %d"%(target_word,rank_list[len(rank_list)-1])

		#for a in range(len(ACC_NUMS)):
			#if target_word in result_list[:ACC_NUMS[a]]:
				#acc_list[a]+=1
		print "%f/%f done and hits: %d"%(cnt,N,hits)
		#if unicode(target_word,'utf-8') in nlp.vocab:
			#rank_list.append(result_list.index(target_word))
	print "The mean rank is: %f"%(np.mean(rank_list))
	acc_list=[float(x)/float(N) for x in acc_list]
	print "The accuracy list is: ",acc_list
